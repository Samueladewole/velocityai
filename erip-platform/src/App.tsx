...
const getSubdomain = () => {
  ...
  if (parts[0] === 'velocity' && parts[1] === 'eripapp') {
    console.log('Detected velocity subdomain');
    return 'velocity';
  }
  
  // Also check for localhost development
  if (hostname === 'localhost' && pathname.startsWith('/velocity')) {
    console.log('Detected velocity path on localhost');
    return 'velocity-dev';
  }
  
  console.log('No velocity subdomain detected');
  return null;
};
...
function App() {
  ...
  if (subdomain === 'velocity' || subdomain === 'velocity-dev') {
    console.log('Rendering VelocityApp');
    return (
      <DateProvider>
        <ScrollToTop />
        <VelocityAppSimple />
  ...
  console.log('Rendering regular ERIP app');
