...
from owasp_security.security_middleware import SecurityMiddleware, SecurityHeadersMiddleware
import uvicorn
import structlog
...
from data_architecture.router import router as data_router
from enhanced_auth.router import router as enhanced_auth_router
...
# Add middleware (order matters - security middleware should be first)
...
async def root():
    ...
    return {
        ...
        "development_mode": {
            "auth_bypass": settings.dev_mode_bypass_auth if settings.environment == "development" else False,
            "default_user": settings.dev_mode_default_user if settings.environment == "development" else None,
            "message": "🚧 Development Mode: Authentication bypassed for testing" if settings.environment == "development" and settings.dev_mode_bypass_auth else "🔒 Production Mode: Authentication required"
        },
        "components": [
...
app.include_router(data_router, prefix="/data", tags=["Data Architecture"])
app.include_router(enhanced_auth_router, prefix="/enhanced-auth", tags=["Enhanced Authentication"])
